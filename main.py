...
connector.execute(
    "CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, DOB TEXT, STREAM TEXT)"
)

# Creating the functions
def reset_fields():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar,id_Sstrvar

    for i in ['id_strvar','name_strvar', 'email_strvar', 'contact_strvar', 'gender_strvar', 'stream_strvar','id_Sstrvar']:
        exec(f"{i}.set('')")
...
def add_record():
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar,id_strvar#new

    sid=id_strvar.get()#new
    ...
    if not name or not email or not contact or not gender or not DOB or not stream or not sid:#new
        mb.showerror('Error!', "Please fill all the missing fields!!")
    else:
        try:
            connector.execute(
            'INSERT INTO SCHOOL_MANAGEMENT (STUDENT_ID, NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (?,?,?,?,?,?,?)', (sid, name, email, contact, gender, DOB, stream)
            )
            connector.commit()
...
def update_record():#new
    global name_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar,id_strvar
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        ...
        try:
            connector.execute(
            'UPDATE SCHOOL_MANAGEMENT SET STUDENT_ID=?, NAME=?, EMAIL=?, PHONE_NO=?, GENDER=?, DOB=?, STREAM=? WHERE STUDENT_ID=?', (sid,name, email, contact, gender, DOB, stream, sid)
            )
            connector.commit()
...
def search_records():
    ...
    if not ids:
        mb.showerror('Error!', "Please fill the ID!!")
    else:
        curr = connector.execute('SELECT * FROM SCHOOL_MANAGEMENT WHERE STUDENT_ID=(?)',(ids,))
        data = curr.fetchall()
        if not data:
            mb.showerror('Error!', "No ID Found!!")
        else:
            for records in data:
                #tree.insert('', END, values=records)
                for i in range(0,7,1):
                    a=a+fields[i]+str(records[i])
                mb.showinfo('Record updated', f"{a}")
                #print(a)
...
id_strvar = StringVar()#new
id_Sstrvar = StringVar()#new
...
Label(main, text="STUDENT INFORMATION SYSTEM", font=headlabelfont, bg='SpringGreen').pack(side=TOP, fill=X)
...
Label(left_frame, text="ID", font=labelfont, bg=lf_bg).place(relx=0.075, rely=0.01)#new
...
Entry(left_frame, width=10, textvariable=id_strvar, font=entryfont).place(x=50, rely=0.01)#new
...
Entry(center_frame, width=19, textvariable=id_Sstrvar, font=entryfont).place(x=20, rely=0.75)#new
...
Button(center_frame, text='Update Record', font=labelfont, command=update_record, width=15).place(relx=0.1, rely=0.55)#new
Button(center_frame, text='Search', font=labelfont, command=search_records, width=15).place(relx=0.1, rely=0.65)#new
Button(center_frame, text='Display', font=labelfont, command=display_records, width=15).place(relx=0.1, rely=0.85)#new
Button(center_frame, text='Clear Screen', font=labelfont, command=reset_form, width=15).place(relx=0.1, rely=0.15)#new
